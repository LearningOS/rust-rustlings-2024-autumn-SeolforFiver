...
/// # Safety
///
/// The `address` must be a valid mutable reference to a `u32` value.
/// It must point to a valid, properly aligned, and initialized memory location 
/// containing a `u32`. The caller is responsible for ensuring the pointer's 
/// validity throughout the modification.
unsafe fn modify_by_address(address: usize) {
    // SAFETY: The caller guarantees that the address is a valid, properly
    // aligned, and initialized pointer to a `u32`, allowing safe dereferencing
    // and modification.
